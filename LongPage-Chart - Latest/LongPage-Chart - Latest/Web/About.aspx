<%@ Page Title="About" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="About.aspx.cs" Inherits="Web.About" %>

<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <div class="col_w900">
        <h2>About Us</h2>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris ipsum felis, consectetur non tincidunt quis, posuere sit amet odio. Suspendisse urna metus, sodales eget sagittis sed, pharetra at lorem. Maecenas eget ante eu magna porttitor commodo. Curabitur sagittis quam ac ante laoreet aliquet.</p>
                
		<div class="cleaner h30"></div>
                
		<img class="image_wrapper image_fl" src="images/tooplate_image_03.jpg" alt="Image 03" />
                
		<h2>Our Services</h2>
        <p>Aliquam varius pulvinar lorem, semper elementum dui sodales non. Donec dictum massa nec tortor auctor molestie. Nulla ornare sagittis nisi nec lacinia. Ut in mauris nibh. Morbi at risus urna. Integer tincidunt, lacus vitae adipiscing aliquam, libero ligula dictum justo, vel malesuada nunc nulla a sapien. Nunc ornare scelerisque est, eget tempor eros placerat eu. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Aliquam nec nisi dolor. Quisque vitae purus et quam auctor ultrices eget vel leo. Mauris egestas euismod venenatis. Validate <a href="http://validator.w3.org/check?uri=referer" rel="nofollow"><strong>XHTML</strong></a> and <a href="http://jigsaw.w3.org/css-validator/check/referer" rel="nofollow"><strong>CSS</strong></a>.</p>
                
		<ul class="tooplate_list">
            <li>Pellentesque ligula tortor, feugiat non sollicitudin a, tristique non magna. </li>
            <li>Cras massa urna, imperdiet vehicula rutrum id, sodales vitae dolor.</li>
            <li>Cras bibendum pharetra orci, sed iaculis tellus sollicitudin eget.</li>
            <li>Vestibulum odio mi, mattis non fermentum et, porttitor ac mauris.</li>
        </ul>
                
        <div class="cleaner"></div>
    </div>
            
    <div class="col_w900 col_w900_last">
        <div class="con_tit_02">Latest Projects</div>
                
		<div class="col_allw280 lp_box">
            <a href="#"><img src="images/tooplate_image_01.jpg" alt="Image 01" /></a>
            <p>Etiam ut nibh et urna cursus ultricies nec vel nunc. In hac habitasse platea dictumst. </p>
            <a class="more" href="#">More</a>
        </div>
                
		<div class="col_allw280 lp_box">
            <a href="#"><img src="images/tooplate_image_02.jpg" alt="Image 02" /></a>
            <p>Donec ullamcorper feugiat nunc ut tempus. Sed sodales, nibh ut ultricies pulvinar.</p>
            <a class="more" href="#">More</a>
        </div>
				
		<div class="col_allw280 lp_box col_last">
            <a href="#"><img src="images/tooplate_image_03.jpg" alt="Image 03" /></a>
            <p>Praesent auctor quam quis libero venenatis dapibus. Donec feugiat nunc ut tempus.</p>
            <a class="more" href="#">More</a>
        </div>
                
		<div class="cleaner"></div>
    </div>
</asp:Content>
